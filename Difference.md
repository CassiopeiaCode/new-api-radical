...
保留面向异步/长任务的全局与用户级槽位控制，同时恢复普通聊天请求的独立活跃度识别和历史观测能力。调度定时任务应在可行处复用最新上游的系统任务调度机制。

### 行为契约

- 默认全局活跃任务槽位上限为 1000、单用户上限为 50，且均应可通过受控配置调整。
- 异步视频/绘图等长任务通过 `Acquire/Release` 占用容量槽；普通 Chat Completions、Completions、Responses、Messages 和 Gemini generateContent 请求只刷新独立 activity profile，绝不能占用异步任务容量，也不能由异步任务污染聊天活跃度。
...
- 管理端实时查询默认使用最近 30 秒窗口，但 `GET /api/active-task/stats?window=<seconds>&limit=<n>` 必须保留旧版可选窗口能力：窗口最大 3600 秒，排名默认 50 条、最大 200 条；两套前端都提供常用窗口选择。每 10 分钟执行一次异常值采样，该采样固定查询最近 600 秒窗口，并仅持久化活跃 profile 数不少于 5 的非管理员用户。部署 Redis 时 activity 使用独立的 `active_task_activity:*` 键跨实例共享，不能与异步容量的 `active_task_slots:*` 键混用。
...
- 热路径状态可保存在内存/缓存，历史观测使用适合查询的持久化模型；异步容量与聊天 activity 两者职责清晰。部署已启用 Redis 时，槽位获取/释放必须使用 Redis 原子协调以在所有实例间落实全局与单用户上限，聊天 activity 也必须跨实例共享；未配置 Redis 时允许单实例内存回退。
