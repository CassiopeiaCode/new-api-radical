...
- 对带完整会话历史的聊天请求，只扫描从倒数第二条 `user` 消息开始到请求末尾的消息后缀，包含其后的 `assistant`、`tool` 和最新 `user` 内容；只有一条 `user` 时从该条开始，无 `user` 或非聊天请求时保留原扫描范围。不得因缩小重复历史扫描而跳过当前轮工具输入、工具结果或独立 prompt。
...
将固定周期 Ping 改为“长空闲阈值 + 短活跃 Ping 间隔”。正常有真实流数据时不注入额外 Ping；只有上游静默超过阈值后，才高频向下游客户端写入 SSE 注释保活。该 Ping 的方向是 relay 到下游客户端，不是向上游供应商发送请求。

### 行为契约

- `ping_idle_threshold_seconds` 表示连续多久没有收到任何上游流数据后进入 Ping 阶段，默认 `90` 秒。
- `ping_interval_seconds` 表示进入 Ping 阶段后的发送间隔，默认 `1` 秒；每次写入的协议内容仍为 `: PING\n\n`，标准 SSE 客户端将其作为注释忽略。
- 任意真实上游流数据到达时立即退出短 Ping 阶段，并从完整空闲阈值重新计时。持续活跃的流不得按固定周期发送 Ping。
...
- 首次 Ping 前应保留足够长的上游正常响应和透明重试窗口，因为一旦向下游写出任何保活字节，后续上游失败通常无法再切换为尚未开始的 HTTP 响应。

### 配置 / API / UI

- `general_setting.ping_interval_enabled`：总开关。
- `general_setting.ping_idle_threshold_seconds`：空闲阈值，必须大于等于 1 秒。
- `general_setting.ping_interval_seconds`：进入空闲阶段后的 Ping 间隔，必须大于等于 1 秒。
- `web/default` 与 `web/classic` 均显示两个独立输入项，并明确说明真实流数据恢复后会重新等待空闲阈值。旧配置缺少空闲阈值时使用后端默认值，不要求数据迁移。
...
- 测试持续有数据的流不会发送 Ping；静默超过阈值后会按短间隔连续发送；真实数据恢复后停止短间隔发送并重新等待阈值。
