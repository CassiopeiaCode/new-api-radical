...
- `/pricing` 模型卡片右下角的三段健康趋势固定表示 `24h / 1h / 5m`，按从左到右排列；每段均以对应窗口内成功请求数除以总请求数计算，窗口无样本时显示灰色。当前生产配置使用 `5min` 桶，因此 `5m` 为当前对齐桶，`1h` 聚合当前及前 11 个桶，`24h` 聚合当前及前 287 个桶；不得再显示为最近三个非空桶。
